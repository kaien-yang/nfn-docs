...
class NPPool(nn.Module):
    def __init__(self, network_spec: NetworkSpec, agg="mean"):
        """
        Args:
            network_spec (NetworkSpec): Network specification.
            agg (str, optional): Type of pooling to perform. One of "mean", "max", or "sum".
                Default: "mean".
        """
        super().__init__()
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> torch.Tensor:
        r"""
        Applies the pooling operation to the input weight space features across any axis that
        has permutation symmetry. The pooling operation is invariant to $`\mathcal{S}`$,
        the neuron permutation (NP) group. See Equation 5 for a complete description.

        Args:
            wsfeat (WeightSpaceFeatures): Input weight space features.

        Returns:
            (torch.Tensor): Output of pooling operation. Shape is $`(B, C, N)`$, where $`N`$ is the
                number of outputs of the global pooling layer.
        """
        out = []
...
class HNPPool(nn.Module):
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> torch.Tensor:
        r"""
        Applies a pooling operation to the input weight space features across any axis that
        has permutation symmetry. The pooling operation is invariant to $`\mathcal{\tilde{S}}`$,
        the hidden neuron permutation (HNP) group. See Equation 20 for a complete description.

        Args:
            wsfeat (WeightSpaceFeatures): Input weight space features.

        Returns:
            (torch.Tensor): Output tensor with shape $`(B, C, N)`$, where $`N`$ is the number of outputs
                of the global pooling layer.
        """
        out = []
...
class Pointwise(nn.Module):
    def __init__(self, network_spec: NetworkSpec, in_channels, out_channels):
		...
		Args:
			network_spec (NetworkSpec): Network specification.
			in_channels (int): Number of input channels of weight space features.
			out_channels (int): Number of input channels of weight space features.
        """
        super().__init__()
        self.network_spec = network_spec
    ...
    def forward(self, wsfeat: WeightSpaceFeatures):
        r"""
        Applies a linear NF-Layer to input weight space features. The layer assumes full row and column
        exchangeability of the weight space features in each layer and ignores interactions between
        the weight space features. Only last term of Equation 3 is used in constructing this layer.

        Args:
            wsfeat (WeightSpaceFeatures): Input weight space features, where each weight and bias has $`C_{in}`$ channels.

        Returns:
            (WeightSpaceFeatures): Output weight space features, where each weight and bias has $`C_{out}`$ channels.
        """
...
class NPLinear(nn.Module):
    def __init__(self, network_spec: NetworkSpec, in_channels, out_channels, io_embed=False):
        ...
        Args:
            network_spec (NetworkSpec): Network specification.
            in_channels (int): Number of input channels of weight space features.
            out_channels (int): Number of output channels of weight space features.
        """
        super().__init__()
    ...
    def forward(self, wsfeat: WeightSpaceFeatures) -> WeightSpaceFeatures:
        r"""
        Applies a linear NF-Layer to input weight space features. The layer is equivariant to $`\mathcal{S}`$,
        the neuron permutation (NP) group. See Equation 3 for a complete description.
...
class HNPLinear(nn.Module):
    ...
    def forward(self, wsfeat: WeightSpaceFeatures):
        r"""
        Applies a linear NF-Layer to input weight space features. The layer is equivariant to $`\mathcal{\tilde{S}}`$,
        the hidden neuron permutation (HNP) group. See Appendix C for a complete description.
